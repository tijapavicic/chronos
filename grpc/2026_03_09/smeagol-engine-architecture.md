# Smeagol Engine Integration Architecture

## Scope

Design for:
- `smeagol-engine-frontend`
- `smeagol-engine-backend`
- `calculation-engine`
- source database (queried by calculation engine)
- smeagol persistence database (written by smeagol backend)
...
## REST Contract (Frontend <-> Smeagol Backend)

### Submit Job
`POST /v1/smeagols/jobs`
...
### Cancel Job
`POST /v1/smeagols/jobs/{jobId}:cancel` -> `202 Accepted`

### Idempotency Rule
- Require `requestId`
- Map `(systemId, requestId) -> jobId` and return same `jobId` on duplicate submit

## gRPC Contract (Smeagol Backend -> Calculation Engine)
...
## Event Contract (Calculation Engine -> Broker -> Smeagol Backend)
...
- High-level flow: see `smeagol-engine-flow.mmd`
- Sequence flow: see `smeagol-engine-sequence.mmd`
